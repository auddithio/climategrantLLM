...
parser.add_argument('--class_accuracy_title', type=str, default='default')
# tweaks!
parser.add_argument('--exclude-label-label-content-pairs', action='store_true')
parser.add_argument('--exclude-label-content-label-content-pairs', action='store_true')
parser.add_argument('--exclude-content-label-pairs', action='store_true')
parser.add_argument('--exclude-content-label-content-pairs', action='store_true')
...
def get_training_pairs_and_loader(titles, contents, min_len, max_len, last_sent_min_len, labels, label_contents,target_indices, batch_size):
    ...
    # label title-label content pairs
    if not args.exclude_label_label_content_pairs:
        with Pool(processes=32) as pool:
            split_contents = pool.map(partial(split_sentence, min_len=min_len, max_len=max_len, last_sent_min_len=last_sent_min_len), label_contents)
            for i in range(len(labels)):
                for c in split_contents[i]:
                    if len(labels[i]) > 0:
                        training_pairs.append((labels[i], c))

    if not args.exclude_label_content_label_content_pairs:
        with Pool(processes=32) as pool:
            split_contents = pool.map(partial(split_sentence, min_len=min_len, max_len=max_len, last_sent_min_len=last_sent_min_len), label_contents)
            for i in range(len(label_contents)):
                len_split_content = len(split_contents[i])
                perm = np.random.permutation(len_split_content)
                for j in range(len_split_content // 2 + (len_split_content % 2 > 0)):
                    if j * 2 + 1 < len_split_content:
    ...
    # Treat labels like titles and create pairs with corresponding contents
    # Pair contents with corresponding labels based on target indices
    if not args.exclude_content_label_pairs:
        with Pool(processes=32) as pool:
            split_contents = pool.map(partial(split_sentence, min_len=min_len, max_len=max_len, last_sent_min_len=last_sent_min_len), contents)
            
            for i, indices in enumerate(target_indices):
                for index in indices:
                    for split_content in split_contents[i]:
                        training_pairs.append((split_content, labels[index]))
    
    # Pair contents with corresponding label contents (Content-Label-Content pairs)
    if not args.exclude_content_label_content_pairs:
        with Pool(processes=32) as pool:
            split_contents = pool.map(partial(split_sentence, min_len=min_len, max_len=max_len, last_sent_min_len=last_sent_min_len), contents)
            split_label_contents = pool.map(partial(split_sentence, min_len=min_len, max_len=max_len, last_sent_min_len=last_sent_min_len), label_contents)

            for i, indices in enumerate(target_indices):
                for index in indices:
                    for split_content in split_contents[i]:
                        for split_label_content in split_label_contents[index]:
                            training_pairs.append((split_content, split_label_content))
...
for epoch in range(args.epochs):
    training_pairs, loader = get_training_pairs_and_loader(
        ...
        lbl_js.title, lbl_js.content, trn_js.target_ind,
    ...
    # print('model', model)
    # print('tst_js.title', tst_js.title)
    # print('tst_content', tst_content)
    # print('lbl_js', lbl_js.title)
    # print('tst_js.target_ind', tst_js.target_ind)
    # print('tst_js.uid', tst_js.uid)
    # print('lbl_js.uid,', lbl_js.uid)

    uid_title_map = {uid: title for uid, title in zip(lbl_js.uid, lbl_js.title)}

    prec = evaluate(
        epoch, args.experiment, uid_title_map, tokenizer, model, tst_js.title, tst_content, lbl_js.title, tst_js.target_ind,
        tst_js.uid, lbl_js.uid,
        batch_size=args.eval_batch_size if isinstance(args.eval_batch_size, int) else args.batch_size * 4, 
        device=device, print_freq=args.print_freq, 
    )
